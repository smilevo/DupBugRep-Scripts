# -*- coding: utf-8 -*-
"""Copie de distillbert.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1alKkrrobXX1sYbxdNnXZII0MzIbDq8z5
...
with torch.no_grad():
    last_hidden_states3 = model(input_ids3, attention_mask=attention_mask3)    # <<< 600 rows only !!!
features3 = last_hidden_states3[0][:,0,:].numpy()
#features3 =last_hidden_states[0][:,0,:]
features3
...
with torch.no_grad():
    last_hidden_states4 = model(input_ids4, attention_mask=attention_mask4)   
...
with torch.no_grad():
    last_hidden_states5 = model(input_ids5, attention_mask=attention_mask5)    # <<< 600 rows only !!!
...
with torch.no_grad():
    last_hidden_states6 = model(input_ids6, attention_mask=attention_mask6)   
...
with torch.no_grad():
    last_hidden_states7 = model(input_ids7, attention_mask=attention_mask7)  
...
with torch.no_grad():
    last_hidden_states8 = model(input_ids8, attention_mask=attention_mask8)   
...
with torch.no_grad():
    last_hidden_states9 = model(input_ids9, attention_mask=attention_mask9)    
...
with torch.no_grad():
    last_hidden_states10 = model(input_ids10, attention_mask=attention_mask10) 
...
with torch.no_grad():
    last_hidden_states11 = model(input_ids11, attention_mask=attention_mask11)   
